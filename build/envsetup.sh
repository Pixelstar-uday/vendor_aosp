...
cat <<EOF
Additional PIXELSTAROS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- pixelstargerrit:   A Git wrapper that fetches/pushes patch from/to PIXELSTAROS Gerrit Review.
...
function pixelstarremote()
{
    ...
    then
        ...
        local PFX="PIXELSTAROS/"
    else
        local PROJECT=$REMOTE
    fi

    local PIXELSTAR_USER=$(git config --get review.review.pixelstaros.org.username)
    if [ -z "$PIXELSTAR_USER" ]
    then
        git remote add pixelstar ssh://review.pixelstaros.org:29418/$PFX$PROJECT
    else
        git remote add pixelstar ssh://$PIXELSTAR_USER@review.pixelstaros.org:29418/$PFX$PROJECT
    fi
...
function pixelstarrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "PIXELSTAROS Gerrit Rebase Usage: "
    ...
    git fetch "http://review.pixelstaros.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
